...
int romanToInteger(string roman) {
  int num = 0;
  std::map<char, int> romanInt{{'M', 1000}, {'D', 500}, {'C', 100}, {'L', 50},
                               {'X', 10},   {'V', 5},   {'I', 1}};
  ...
  for (int i = 0; i < roman.length(); i++) {
    ...
    itr = romanInt.find(roman[i]);
    if (itr != romanInt.end()) {
      currChar = romanInt[roman[i]];
    }
    itr = romanInt.find(roman[i + 1]);
    if (itr != romanInt.end()) {
      nextChar = romanInt[roman[i + 1]];
    }
    if (currChar >= nextChar) {
      num = num + currChar;
    } else if (currChar < nextChar) {
      num = num + (nextChar - currChar);
      ++i;
    }
  }
  return num;
